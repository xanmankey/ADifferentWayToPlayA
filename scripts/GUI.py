...
config_object.read("configs/config.ini")

# Setting configuration settings
discordConfig = config_object["DISCORD"]
randomConfig = config_object["RANDOM"]
miopConfig = config_object["MIOP"]
...
def create_window1():
    layout = [[sg.Text('Discord Text to Game Input', font=('Comic Sans MS', 24), pad=(160, 0))],
              ...
              [sg.Text('Bot Invite Link', pad=(330, 10), enable_events=True,
              ...
              [sg.Checkbox("Up", pad=(25, 0), key="1", default=eval(discordConfig["up"])), sg.Checkbox("LightUp", key="2", default=eval(discordConfig["lightup"])), sg.Checkbox("Down", key="3", default=eval(discordConfig["down"])), sg.Checkbox("LightDown", key="4", default=eval(discordConfig["lightdown"])), sg.Checkbox(
                  "Left", key="5", default=eval(discordConfig["left"])), sg.Checkbox("LightLeft", key="6", default=eval(discordConfig["lightleft"])), sg.Checkbox("Right", key="7", default=eval(discordConfig["right"])), sg.Checkbox("LightRight", key="8", default=eval(discordConfig["lightright"])), sg.Checkbox("Lump", key="9", default=eval(discordConfig["lump"]))],
              [sg.Checkbox("Rump", pad=((30, 0), (0, 0)), key="10", default=eval(discordConfig["rump"])), sg.Checkbox("Start", key="11", default=eval(discordConfig["start"])), sg.Checkbox("RESET", key="12", default=eval(discordConfig["reset"])), sg.Checkbox("A", key="13", default=eval(discordConfig["a"])), sg.Checkbox(
                  "HoldA", key="14", default=eval(discordConfig["holda"])), sg.Checkbox("B", key="15", default=eval(discordConfig["b"])), sg.Checkbox("HoldB", key="16", default=eval(discordConfig["holdb"])), sg.Checkbox("X", key="17", default=eval(discordConfig["x"])), sg.Checkbox("LThumb", key="18", default=eval(discordConfig["lthumb"])), sg.Checkbox("Y", key="19", default=eval(discordConfig["y"])), sg.Checkbox("Select", key="20", default=eval(discordConfig["select"]))],
              [sg.Checkbox("LT", pad=((60, 0), (0, 0)), key="21", default=eval(discordConfig["lt"])), sg.Checkbox("HoldLT", key="22", default=eval(discordConfig["holdlt"])), sg.Checkbox("RT", key="23", default=eval(discordConfig["rt"])), sg.Checkbox("HoldRT", key="24", default=eval(discordConfig["holdrt"])), sg.Checkbox(
                  "LB", key="25", default=eval(discordConfig["lb"])), sg.Checkbox("RB", key="26", default=eval(discordConfig["rb"]))]
              ]
...
def create_window2():
    layout = [[sg.Text('Randomized Controls', font=('Comic Sans MS', 24), pad=(190, 0))],
              ...
              [sg.Radio('Axis Randomization', "Radio", pad=((150, 0), (0, 0)), enable_events=True, default=eval(randomConfig['axis'])), sg.Radio(
                  'Button Randomization', "Radio", enable_events=True, default=eval(randomConfig['button'])), sg.Radio('Both Randomized', "Radio", enable_events=True, default=eval(randomConfig['both']))],
...
def create_window3():
    layout = [[sg.Text('Multiple Players One Controller', font=('Comic Sans MS', 24), pad=(135, 0))],
              ...
              [sg.Radio('All Together', "miopRadio", pad=((250, 0), (0, 0)), key=("allTogether"), default=eval(miopConfig['alltogether']), enable_events=True), sg.Radio(
                  'Teams', "miopRadio", pad=((25, 0), (0, 0)), enable_events=True, key=("teams"), default=eval(miopConfig['teams']))],
              [sg.Text('                   Team1                                    Team2', font=(
                  'Comic Sans MS', 18), pad=(20, 0), key="teamsText", visible=False)],
              [sg.Column([[sg.Checkbox("1", key="1", pad=((100, 0), (0, 0)), visible=False, default=eval(miopConfig['p1-1']))]]), sg.Column([[sg.Checkbox("2", key="2", visible=False, default=eval(miopConfig['p1-2']))]]), sg.Column([[sg.Checkbox("3", key="3", visible=False, default=eval(miopConfig['p1-3']))]]), sg.Column([[sg.Checkbox("4", key="4", visible=False, default=eval(miopConfig['p1-4']))]]), sg.Column(
                  [[sg.Checkbox("1", key="5", pad=((120, 0), (0, 0)), visible=False, default=eval(miopConfig['p2-1']))]]), sg.Column([[sg.Checkbox("2", key="6", visible=False, default=eval(miopConfig['p2-2']))]]), sg.Column([[sg.Checkbox("3", key="7", visible=False, default=eval(miopConfig['p2-3']))]]), sg.Column([[sg.Checkbox("4", key="8", visible=False, default=eval(miopConfig['p2-4']))]])]
              ]
...
while True:
    ...
    if window1:
        ...
        if event == "INVITE":
            webbrowser.open(
                "https://discord.com/api/oauth2/authorize?client_id=926229517217976390&permissions=292057851904&scope=bot")
        ...
        if event == 'DISCORD':
            if color1 == 'red':
                ...
                if len(textID) != 18:
                    ...
                    color1 = 'red'
                    error = 1
                    # Exit the if statements to ignore the call to run the file
                for j in range(26):
                    commands[j] = values[str(j + 1)]
                    if commands[j] is True:
                        numCommands += 1
                if numCommands == 0:
                    sg.popup("You must have more than one command enabled!")
                    window['DISCORD'].update(button_color=('red'))
                    color1 = 'red'
                    error = 2
                    # Exit the if statements to ignore the call to run the file
                ...
                print("" + str(commands))
                if error == 0:
                    textCommands = subprocess.Popen(
                        [sys.executable, "TC.py", textID, str(commands)], stdout=subprocess.PIPE, shell=False)
            elif color1 == 'green':
                window['DISCORD'].update(button_color=('red'))
                color1 = 'red'
                textCommands.kill()
        ...
        if event == 'RANDOM':
            if color2 == 'red':
                ...
                # CLI arguments are the first parameter w/ subprocess.Popen. CL arguments need to be STRINGS
                # Also, sys.executable is just the path to python.exe on the users
                randomInputs = subprocess.Popen([sys.executable, "RC.py", randomization, str(
                    seconds)], stdout=subprocess.PIPE, shell=False)
            elif color2 == 'green':
        ...
        if event == 'MIOP':
            if color3 == 'red':
                ...
                if values["allTogether"] is True:
                    ...
                    gamemode = "teams"
                miop = subprocess.Popen([sys.executable, "MIOP.py", gamemode, str(
                    players)], stdout=subprocess.PIPE, shell=False)
            elif color3 == 'green':
